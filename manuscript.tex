...
This is not a limitation of current neuroscience but a structural feature of oscillatory code-forming systems. The ``art'' of clinical judgment, the irreducibility of expert intuition, the limits of algorithmic psychiatry---these may reflect not ignorance but genuine incompleteness.
...
\begin{itemize}
    ...
    \item \texttt{godel\_oscillator.py}: Self-referential oscillator implementation (Appendix B)
\end{itemize}
...
\caption{The self-referential (``Gödel'') oscillator. Top: phase evolution showing irregular dynamics as the system avoids its own predictions. Middle: frequency modulation driven by self-referential feedback. Bottom: resulting symbol sequence. The system maintains an n-gram model of its symbol history, predicts its next output, then adjusts frequency to evade that prediction---physically instantiating ``this system does not behave as its model predicts.''}
\label{fig:godel}
\end{figure}
