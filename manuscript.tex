...
% See separate title page for author details
...
\noindent Mathematically, this framework applies to any continuous dynamical system with a coarse-graining partition; we retain the term ``oscillatory'' because our primary physical examples---neural, physiological, cellular---are oscillation-dominated systems.
...
A methodological note: the very language of ``systems''---with inputs, outputs, states, and transitions---may subtly beg the question by framing dynamics in already-symbolic terms. If symbols emerge from pre-symbolic dynamics through dimensional bottlenecks, then ``system'' language presupposes what needs to be explained. This suggests caution when applying systems-theoretic frameworks to living things: the framework may import symbolic structure that the organism has not yet formed.\footnote{This is especially relevant in neuroscience and biology, where ``information-processing system'' has become the default framing. Our analysis suggests this framing may obscure precisely the phenomenon it aims to explain.}
...
% Acknowledgments, Funding, and Declarations moved to separate Title Page for double-blind review
...
\begin{itemize}
    ...
    \item Suppose $T \vdash \neg G$. Then $T \vdash \text{Prov}_T(\ulcorner G \urcorner)$. But if $T$ is arithmetically adequate (Definition 3), this means $G$ is provable, so $T \vdash G$, contradiction.
\end{itemize}
